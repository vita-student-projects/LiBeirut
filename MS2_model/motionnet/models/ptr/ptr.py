...
class PTR(BaseModel):
    ...
    def _forward(self, inputs):
        ...
        max_shift_x = (np.random.random()-0.5)/2.5
        max_shift_y = (np.random.random()-0.5)/2.5
        ...
        # constant shift on first 5 steps
        constant_shift_x = np.full(rows, max_shift_x)
        constant_shift_y = np.full(rows, max_shift_y)

        ego_in[:,:rows,0] += constant_shift_x
        ego_in[:,:rows,1] += constant_shift_y
