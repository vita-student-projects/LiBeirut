...
class PTR(BaseModel):
    ...
    def _forward(self, inputs):
        ...
        # max_shift_x = (np.random.random()-0.5)/4
        # max_shift_y = (np.random.random()-0.5)/4

        # # rows = ego_in[0].shape[0]
        # batches = ego_in[0].shape[0]
        # # print("batches: ", batches)

        # rows = 5 #shift over the first 5 timesteps
        # # print("OG ego: ", ego_in[8,:,:])

        # # Create a linearly decreasing vector
        # decreasing_shift_x = np.linspace(max_shift_x, 0, rows)
        # # print("added x shift: ",decreasing_shift_x )

        # decreasing_shift_y = np.linspace(max_shift_y, 0, rows)

        # ego_in[:,:rows,0] += decreasing_shift_x
        # ego_in[:,:rows,1] += decreasing_shift_y
               
        # ego_in[:,:,0] *= ego_in[:,:,2]
        # ego_in[:,:,1] *= ego_in[:,:,2]
        # # print("final ego: ", ego_in[8,:,:])
    ...
    def configure_optimizers(self):
        print("ptr parameters: ", self.parameters())
