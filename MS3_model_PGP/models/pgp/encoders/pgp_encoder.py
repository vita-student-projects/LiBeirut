from models.pgp.encoders.encoder import PredictionEncoder
import torch
...
class PGPEncoder(PredictionEncoder):
    ...
    def __init__(self, args: Dict):
        ...
        print(",,,,,,,,,,,,,,,,,")
        print("args['nbr_feat_size'] : ", args['nbr_feat_size'])
        print("args['nbr_emb_size']: ", args['nbr_emb_size'])
    ...
    def forward(self, inputs: Dict) -> Dict:
        ...
        print("****** ", nbr_vehicle_feats.shape)
        nbr_vehicle_masks = inputs['surrounding_agent_representation']['vehicle_masks']
        print("..................")
        print("nbr_vehicle_feats: ", nbr_vehicle_feats.shape)
        ...
        print("finished surrounding agents")
        ...
        print("finished agent-node attention")
    ...
    def variable_size_gru_encode(feat_embedding: torch.Tensor, masks: torch.Tensor, gru: nn.GRU) -> torch.Tensor:
        ...
        masks_for_batching = ~masks[:, :, :].bool()
        print("///////////")
        print("masks_for_batching BEFORE: ", masks_for_batching.shape)

        # masks_for_batching = masks_for_batching.any(dim=-1).unsqueeze(2).unsqueeze(3)
        masks_for_batching = masks_for_batching.permute(1,0,2)

        print("masks_for_batching AFTER: ", masks_for_batching.shape)
        print("feat_embedding BEFORE: ", feat_embedding.shape)
        feat_embedding= feat_embedding.permute(2,1,0,3)
        print("feat_embedding AFTER: ", feat_embedding.shape)

        feat_embedding_batched = torch.masked_select(feat_embedding, masks_for_batching)
        
        feat_embedding_batched = feat_embedding_batched.view(-1, feat_embedding.shape[0], feat_embedding.shape[3])
        print("feat_embedding_batched: ", feat_embedding_batched.shape)

        # Pack padded sequences
        print("masks: ", masks[:,:,:])
        # seq_lens = torch.sum(1 - masks[:, :, :], dim=-1)
        ...
        if len(seq_lens_batched) != 0:
            ...
            # Encode
            print("feat_embedding_packed: ", feat_embedding_packed)
